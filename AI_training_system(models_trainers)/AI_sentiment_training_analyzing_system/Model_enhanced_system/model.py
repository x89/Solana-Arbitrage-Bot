...
def create_targets(df, future_periods=6, profit_threshold=0.015, stop_loss=-0.01):
    """
    Create target labels with RSI confirmation
    """
    df = df.copy()
    ...
    # Buy signals - price will rise significantly AND RSI not overbought
    buy_condition = (returns > profit_threshold) & (df['RSI'] < 65)
    df.loc[buy_condition, 'TARGET'] = 1
    
    # Sell signals - price will fall significantly AND RSI not oversold
    sell_condition = (returns < stop_loss) & (df['RSI'] > 35)
...
def backtest(df, predictions, initial_capital=10000, commission=0.0005):
    """
    Comprehensive backtesting function with proper buy-and-hold comparison
    ...
    for i in range(len(df)):
        ...
        # Execute buy signal (only if we have capital and price is valid)
    ...
    results = {
        ...
        'max_drawdown': (min(portfolio_values) - max(portfolio_values)) / max(portfolio_values) * 100 if portfolio_values else 0,
...
def main():
    ...
    test_data = create_targets(test_data)
    ...
    scaler = RobustScaler()  # More resistant to outliers than StandardScaler
    ...
    test_data = test_data.iloc[:len(test_pred)]  # Align lengths
    results = backtest(test_data, test_pred, initial_capital=10000)

    print_results = print_backtest_results(results)
    ...
    # print(f"\nTrade Statistics:")
    # print(f"Total Trades: {results['trade_details']['total']}")
    # print(f"Winning Trades: {results['trade_details']['winners']}")
    # print(f"Losing Trades: {results['trade_details']['losers']}")
    # print(f"Win Rate: {results['trade_details']['win_pct']:.2f}%")
    # print(f"Average Win: {results['avg_win_pct']:.2f}%")
    # print(f"Average Loss: {results['avg_loss_pct']:.2f}%")
    # print(f"Profit Factor: {results['trade_details']['profit_factor']:.2f}")
    ...
    # Plot results
    plot_results(test_data, {**results, 'initial_capital': initial_capital})
    print("\nSaved backtest visualization to 'backtest_results.png'")
    
    # Save model
    os.makedirs('model', exist_ok=True)
    with open('model/trading_model.pkl', 'wb') as f:
        pickle.dump(model, f)
